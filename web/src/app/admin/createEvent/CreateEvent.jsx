    ...
    <div>
      <Form onFinish={handleSave}>
        <Form.Item label="Event Name" name="eventName">
          <Input name="eventName" onChange={handleChange} placeholder="Enter the event name" />
        </Form.Item>
        <Form.Item label="Event Location" name="eventLocation">
          <Input name="eventLocation" onChange={handleChange} placeholder="Enter the event location" />
        </Form.Item>
        <Form.Item label="Event Details" name="eventDetails">
          <TextArea rows={4} name="eventDetails" onChange={handleChange} placeholder="Describe the event details" />
        </Form.Item>
        <Form.Item label="Event Time" name="eventTime">
          <TimePicker onChange={(time) => handleDateTimeChange(time, 'eventTime')} />
        </Form.Item>
        <Form.Item label="Event Date" name="eventDates">
          <RangePicker onChange={(dates) => handleDateTimeChange(dates, 'eventDates')} />
        </Form.Item>
        <Form.Item label="Upload Flyer" name="eventFlyer" valuePropName="fileList" getValueFromEvent={normFile}>
          <Upload
